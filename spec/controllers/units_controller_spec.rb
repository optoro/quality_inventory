...
RSpec.describe UnitsController, type: :controller do
  ...
  let(:valid_attributes) {
    FactoryBot.attributes_for(:unit).merge(product_id: product.id)
  }
  ...
  describe "PUT #update" do
    context "with valid params" do
      let(:new_attributes) {
        FactoryBot.attributes_for(:unit)
      }
      ...
      it "updates the requested unit" do
        ...
        unit.reload
        expect(unit.price_cents).to eq(new_attributes[:price_cents])
        expect(unit.condition).to eq(new_attributes[:condition])
        expect(unit.status).to eq(new_attributes[:status])
      end
